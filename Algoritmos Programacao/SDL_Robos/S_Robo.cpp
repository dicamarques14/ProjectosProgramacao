...
void ObstaculosMatriz(Matriz *M)
{
	...
	for (int i = 0; i < obst; i++)
	{
		...
		j = Randomizer(0, Tamanho-1);  //gerar um numero aleatorio limitado pelo tamanho da matriz
		printf("%d;", j);
...
void GeraPosicao(Robo *R1, Matriz *M)
{
	...
	do
	{
		...
		i = Randomizer(0, Tamanho-1); //gerar um numero aleatorio limitado pelo tamanho da matriz
		//wait(1000); //Esperar 1s caso contrario gera numeros repetidos
...
void Bateria(Robo *R1)
{
	...
	int bat;
	bat = Randomizer(2,8); //gera valor entre 8 e 2
...
Robo_Struct*  initRoboGUI(){
	return nullptr;
}

Robo_Struct*  initRobo(){
	Robo_Struct *Robozito =  (Robo_Struct *)malloc(sizeof(Robo_Struct));
	if (Robozito)
	{
		int  mX = 0, mY = 0;
		do {
		...
		Robozito->M = AlocarMatriz(mX, mY);	//Preparar a matriz, alocar na memoria, colocar obstaculos

		PrintMatriz(Robozito->M); printf("\n");
		//MATRIZ FEITA

		Robozito->Imperio = AlocarImperio();
		for (int i = 0; i < Robozito->Imperio->QtdRobo; i++)
		{
			printf("Robo %d: ", i + 1);
			IniciaRobo(Robozito->M, &Robozito->Imperio->R[i]); //iniciar o 1º robo, gerar o seu destino, o seu inicio e colocar os valores predefinidos (bateria, activo etc.)
		}

	}

	//ROBO PRONTO
	if(Robozito->M && Robozito->Imperio){
		return Robozito;
	}
...
void main_Robo(Robo_Struct* RoboData)
{
	...
	do
	{
		...
		RoboData->M = AlocarMatriz(mX, mY);	//Preparar a matriz, alocar na memoria, colocar obstaculos
		PrintMatriz(RoboData->M); printf("\n");
		//MATRIZ FEITA

		RoboData->Imperio = AlocarImperio();
		for (int i = 0; i < RoboData->Imperio->QtdRobo; i++)
		{
			printf("Robo %d: ", i + 1);
			IniciaRobo(RoboData->M, &RoboData->Imperio->R[i]); //iniciar o 1º robo, gerar o seu destino, o seu inicio e colocar os valores predefinidos (bateria, activo etc.)
		}
		//ROBO PRONTO
		*/
		Exit = Simulacao(RoboData->Imperio, RoboData->M);
